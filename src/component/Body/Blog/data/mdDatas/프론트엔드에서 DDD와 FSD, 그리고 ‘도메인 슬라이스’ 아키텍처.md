...
- 그리고 어떤 분이 Velog에 정리해둔 “프론트엔드 도메인 아키텍처” 글을 같이 보게 됐다.
...
- **과도한 공통화**
  - `CardView` 하나에 포스트/상품/유저카드의 레이아웃을 다 때려넣기 →
    분기(if/삼항연산자) 지옥
- **“관심사 분리”의 역설**
...
Velog 글은 Redux의 Slice구조를 참고해서 FSD 방식에 적용하여 **조금 더 실전적으로 단순화한 구조**를 제안한다.
...
- `entities` / `features` / `processes` 경계가 팀 합의 없이 흔들리면,
  - **폴더 구조가 오히려 의사결정 비용이 되고 “어디 두지?”가 반복된다.**
- public API / import rule 없이 slice 간 직접 참조가 늘면
  - ui/model/api가 레이어를 가로질러 퍼져 보이고 변경 위치 추적이 어려워진다.
...
## 6. 앞으로 설계할 때 쓸 수 있는 실전 사항(?)

마지막으로, 실제 프로젝트에서 써먹을 수 있는 방법들을 정리해보자.
...
어느 방법이든 정답은 없다. 각자 현 상황에 최선인 방법들을 고르는 것이 맞을 것이다.
