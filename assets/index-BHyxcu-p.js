...
`,jv=`# JS 이벤트 루프를 이해하려면, 먼저 세 개의 레이어부터 갈라야 한다 (1편)\r
\r
...
`,Nv=`# React × Zustand × Closure: \`useCallback\` 쓰다가 stale closure에 뚝배기 맞은 썰 (그리고 구조적으로 막는 법)\r
\r
...
`,Mv=`# React 라이브러리 만들고 “테스트 + CI + 자동 배포” 붙이기 (찐 삽질기 / EOTP 지옥 포함)\r
\r
...
\r
(그리고 이게 제일 좋다)\r
`,_v=`# React 실시간 Trigger 처리 최적화 삽질기\r
\r
...
`,Dv=`# [고찰] “AI로 업무 전체 자동화하면 끝 아니야?”이라는 시니어를 보며 느낀 찝찝함의 정체
...
---`,Rv=`# 브라우저 안에서 JS와 렌더링은 어떻게 돌아가는가 (2편)\r
\r
...
`,zv=`# 커링?? 컬링도 아니고 이게 뭐누 (근데 실무에서 은근 자주 만남)\r
\r
개발하다 보면 이런 순간이 있다.\r
\r
- 함수 하나 만들었는데\r
...
`,Uv=`# 프론트엔드에서 DDD와 FSD, 그리고 ‘도메인 슬라이스’ 아키텍처\r
\r
